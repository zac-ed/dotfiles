...
keys = [
    ...
    Key(                     ### Calls a config-switch script ###
        [mod, "shift"], "r", ### I'm using a cheap 4K Tv that ###
        lazy.spawn(          ###     gets burned in easily    ###
        'switch'
                  ), 
        desc="config switcher"
       ),
    Key(           
        [mod], "o",
        lazy.spawn(
        'xmodmap ~/.Xmodmap'
                  ),            ### Remapping F Keys for OSRS comfort ### 
        desc="Change Keybinds for F1, F2, F3, to zxc"
        ),
    Key(           
        [mod, "shift"], "o",
        lazy.spawn(
        'undo_xmodmap'
                  ),            ###  Undo the remap, for ease of use  ###
        desc="reverse change keybinds for F1, F2, F3, to zxc by calling a script"
        ), 
            ]    
